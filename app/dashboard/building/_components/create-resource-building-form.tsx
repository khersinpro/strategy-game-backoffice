...
import { createResourceBuilding } from "@/src/service/resource-building"
...
export default function CreateResourceBuildingForm({ isSelected = true, token }: { isSelected: boolean, token: string }) {
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        try {
            ...
            setLoading(true)
            setSuccess(false)
            await createResourceBuilding(token, { name, resource_name: selectResource })
            setSuccess(true)
        }
        catch (error: any) {
    ...
    return (
        ...
        <form onSubmit={handleSubmit} className="grid gap-4 md:gap-6">
            ...
            <CustomSelectFormField
                label="Type de ressource produite"
                placeholder="Selection du type de ressource produite"
            ...
            <SuccessAlert 
                ...
                message="Le bâtiment de resource a été créé avec succès" 
            />
            <ErrorAlert 
            ...
            <Button type="submit" disabled={loading}>
                ...
                Créer le bâtiment de ressource
            </Button>
        </form>
    )
}