...
import { CivilizationCreateSchema } from "@/src/schemas/civilization"
...
export default function CreateCivilizationForm() {
    ...
    const handleSubmit = useCallback( async (e: React.FormEvent<HTMLFormElement>) => {
        try {
            ...
            setErrors({})
            setSuccess(false)
            await axios.post(`${process.env.API_URL}/civilization`, 
                {
                    name
                },
                {
    ...
    return (
        <div className="flex justify-center">
            <Card className="w-[450px] grid gap-4 p-4">
                <CardTitle>Créer une civilisation</CardTitle>
                <form onSubmit={handleSubmit} className="grid gap-4">
                    <SuccessAlert 
                        isSuccess={success} 
                        title="Opération réussie" 
                        message="La civilisation a été créée avec succès" 
                    /> 
                    <ErrorAlert 
